...
public class BlockInvertedLamp extends BlockColored implements IConnectedTexture {
	@SideOnly(Side.CLIENT)
	private Icon[][] blockIcons;

	public BlockInvertedLamp(int id, String name, String readableName){
		super("au_extras", id, name, readableName, ItemBlockInvertedLamp.class, Material.glass);
	}
	...
	public void registerIcons(IconRegister iconRegister){
		...
		for(int c = 0; c < 16; c++)
			for(int t = 0; t < 47; t++)
				this.blockIcons[c][IConnectedTexture.ctm[t]] = iconRegister.registerIcon(this.modPath+this.getUnlocalizedName().replace("tile.au.", "").replace("Inverted", "")+c+"-"+t);
	}
	...
	public boolean canConnectTextures(int id, int meta, int side, BlockCoord neighbor){
		// must be same color of inverted lamp to connect
		if(neighbor.getBlockID() != id) return false;
		if(neighbor.getBlockMetadata() != meta) return false;

		// must not have same color of inverted lamp on this side (to render inner frames)
